...
def draw_band(ax, xc, y, w, h, strength):
    ax.add_patch(Rectangle((xc - w/2, y - (h*1.8)/2), w, h*1.8,
                           facecolor="white", edgecolor="none", alpha=0.08 * strength))
    ax.add_patch(Rectangle((xc - w/2, y - h/2), w, h,
                           facecolor="white", edgecolor="none", alpha=0.70 + 0.25 * strength))

ladder_bp = list(range(100, 1100, 100)) + [1500, 2000, 2500, 3000]
ladder_bp = [bp for bp in ladder_bp if bp_min <= bp <= bp_max]

lane_labels = ["M"] + list(enzymes.keys())
lane_count = len(lane_labels)

fig, ax = plt.subplots(figsize=(11.5, 4.7), dpi=300)
ax.set_xlim(0, 1)
ax.set_ylim(0, 1)
ax.axis("off")
fig.subplots_adjust(left=0.07, right=0.985, top=0.86, bottom=0.25)

gel_x0, gel_y0 = 0.10, 0.20
gel_w, gel_h = 0.88, 0.62

ax.add_patch(Rectangle((gel_x0, gel_y0), gel_w, gel_h,
                       facecolor="#070707", edgecolor="black", linewidth=2.4))
...
for i in range(lane_count):
    ...
    ww = lane_w * 0.64
    ax.add_patch(Rectangle((wx, well_y), ww, well_h, facecolor="#111111", edgecolor="none"))

for i in range(1, lane_count):
    x = gel_x0 + lane_w * i
    ax.plot([x, x], [gel_y0, gel_y0 + gel_h], linewidth=0.8, alpha=0.07, color="white")

band_h = gel_h * 0.012
band_w_ladder = lane_w * 0.56
band_w_sample = lane_w * 0.62

for bp in ladder_bp:
    y = gel_y0 + gel_h * y_from_bp(bp)
    strength = 1.0 if bp in (500, 1500) else 0.8
...
for lane_i, enz in enumerate(enzymes.keys(), start=1):
    ...
    for L in frags:
        ...
        frac = L / max(1, n)
        strength = 0.65 + 0.45 * (frac ** 0.35)
        strength *= 0.9 + 0.2 * random.random()

        draw_band(ax, lane_centers[lane_i], y, band_w_sample, band_h, strength)

for i, label in enumerate(lane_labels):
    ax.text(lane_centers[i], gel_y0 - 0.065, label, ha="center", va="top")

scale_x = gel_x0 - 0.040
ax.plot([scale_x, scale_x], [gel_y0, gel_y0 + gel_h], linewidth=3.0, color="black")
for bp in (3000, 1500, 500, 100):
    y = gel_y0 + gel_h * y_from_bp(bp)
    ax.plot([scale_x - 0.012, scale_x + 0.012], [y, y], linewidth=3.0, color="black")
    ax.text(scale_x - 0.018, y, f"{bp} bp", ha="right", va="center")

ax.text(gel_x0, gel_y0 - 0.15, "Top (wells)", ha="left", va="top")
ax.text(gel_x0, gel_y0 - 0.2, "↓ smaller fragments migrate further", ha="left", va="top")

ax.set_title("Simulated gel electrophoresis (restriction digests)", fontsize=22, pad=12)

plt.savefig("Screenshot_4.jpg", dpi=300, bbox_inches="tight", pad_inches=0.10)
plt.close()
